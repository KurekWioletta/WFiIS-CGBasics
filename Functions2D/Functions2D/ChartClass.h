...
private:
	...
	void line2d(wxDC* dc, Matrix matrix, double x1, double y1, double x2, double y2, int h);
	// powyzej sugerowana funkcja transformujaca odcinki (jesli ktos ma 
	// inny pomysl mozna ja usunac)

	/* added methods */
	Matrix translateMatrix() const;
	Matrix rotateMatrix(int w, int h) const;
	Matrix scaleMatrix(int w, int h) const;
	wxPoint convertPoint(Matrix t, float x, float y);
	void DrawChartAxis(Matrix matrix, wxDC * dc, int h);
	void DrawChartFunction(Matrix matrix, wxDC * dc, int h);
public:
